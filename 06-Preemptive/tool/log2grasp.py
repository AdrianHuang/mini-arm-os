...
for line in lines :
	...
	if inst == 'task' :
		...
		out_time = (int(tick) + (int(tick_reload) - int(out_minitick)) / float(tick_reload)) / 10 * 1000;
		in_time  = (int(tick) + (int(tick_reload) - int(in_minitick))  / float(tick_reload)) / 10 * 1000;
		...
		out_task_info = tasks[out_task]
		in_task_info = tasks[in_task]
